...
| ライフサイクル       | `api/api/lifespan.py`               | 起動時マップ・ユーザー読込、`RoomContext` 初期化、ハンドラー登録（`register(ctx)`）、`mixing_loop(ctx)` タスク管理    |
| RTC状態コンテキスト  | `api/rtc/state.py`                  | `RoomContext` dataclass。`active_sessions` / `muted_users` / `current_islands` / `audio_tasks` / handlers を集約      |
| RTC / 音声ミックス   | `api/rtc/rtc.py`                    | LiveKit ボット、10ms オーディオフレーム、アイランド（島）グルーピング                                                 |
| コマンド定義         | `api/rtc/adapter.py`                | `HostCommand`/`GuestCommand` enum、`Command` 継承の payload dataclass、`RoomContext` を受ける `send_message` ヘルパー |
| リアルタイムコマンド | `api/master/master.py`              | `register(ctx)` で登録する MOVE / UPDATE / MUTE ハンドラー。MOVE 受信時に即時 MOVED ブロードキャスト（イベント駆動）  |
...
| メインコンテナ       | `src/main/Main.tsx`                 | `createManager()` と `/init` を扱い、状態を `MainView.tsx` に橋渡しする container                                     |
| メインビュー         | `src/main/MainView.tsx`             | Presentational component。canvas/audio ref と `VoicePanel` / `ProfileForm` の表示を担当                               |
...
| Manager ファクトリ   | `src/main/createManager.ts`         | `Main.tsx` から参照する生成境界。container test で差し替えるポイント                                                  |
...
- Container と presentational component を分けられる箇所では分離を優先し、`Main.tsx` のような配線コードと `MainView.tsx` のような表示コードを混在させないでください。
...
- RTC の mutable state は `api/rtc/state.py` の `RoomContext` に集約し、`init_room(ctx, ...)` / `mixing_loop(ctx)` / `register(ctx)` のように明示的に渡します。
- LiveKit/GuestCommand ハンドラー登録には `@on_message(ctx)` / `@on_join(ctx)` / `@on_leave(ctx)` デコレータパターンを使用します。
...
| `tests/ui/MainView.test.tsx`             | `MainView` の表示テスト。接続状態、ユーザー一覧、ミュート表示                    |
| `tests/ui/Viewer.test.tsx`               | `Main` container の配線テスト。`/init`、manager connect/disconnect、state bridge |
...
- **バックグラウンドタスク**: 通常テストは `lifespan` を持たない `test_app` フィクスチャを使用し `mixing_loop(ctx)` を起動しない。LiveKit 統合テストは必要に応じて `asyncio.create_task(mixing_loop(room_context))` で起動する
- **ハンドラー登録**: `api.master.master.register(ctx)` を lifespan 起動時と `tests/conftest.py` の `room_context` フィクスチャから明示的に呼び出す
...
- **コンテナ境界のモック**: `Main.tsx` のテストでは `src/main/createManager.ts` をモックし、表示テストは `MainView.tsx` に寄せる。`Manager` クラス自体を UI 表示テストで丸ごとモックしない

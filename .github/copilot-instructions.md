...
| エリア               | 主要ファイル           | 内容                                                                                           |
| :------------------- | :--------------------- | :--------------------------------------------------------------------------------------------- |
| APIルート            | `api/api/router.py`    | `/api/init`, `/api/discord`, `/api/token`, `/api/master`                                       |
| 認証                 | `api/api/auth.py`      | HMAC-SHA256 セッションCookie + JWT LiveKit トークン                                            |
...
| リアルタイムコマンド | `api/master/master.py` | MOVE / UPDATE / MUTE ハンドラー、`send_message_others` による送信者除外                        |
| マップ解析           | `api/master/mapper.py` | TMXレイヤーからのシードフィルによるアイランドのラベリング                                      |
| ユーザーモデル       | `api/master/user.py`   | `User` Pydantic モデル、`UserStore` クラス                                                     |
...
| メインビュー         | `src/viewer/Main.tsx`  | `ViewerManager.ts` と連携                                                                      |
...
# テスト用ボット追加（サーバーと LiveKit 起動済みの状態で実行）
uv run tools/add_bots.py <N>  # N 体のボットが参加・移動し方形波音声を送信。Ctrl+C で終了
